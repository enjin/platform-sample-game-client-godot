...
func mint_token(user_auth: String, token_id: String, amount: int) -> bool:
    return await _bool_token_op(user_auth, "mint", {"tokenId": token_id, "amount": amount})


func melt_token(user_auth: String, token_id: String, amount: int) -> bool:
    return await _bool_token_op(user_auth, "melt", {"tokenId": token_id, "amount": amount})
...
func get_managed_wallet_tokens(user_auth: String) -> PlatformModels.ManagedWalletAccount:
    ...
    if result.status_code == 401:
        ...
        if Engine.has_singleton("EnjinManager"):
            EnjinManager.logout()
        else:
            # Autoload reference works at runtime; this branch is just defensive
            # in case the API service is used outside the normal app.
            pass
        return null
    if not result.ok:
